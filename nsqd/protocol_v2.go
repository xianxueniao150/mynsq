...
func (p *protocolV2) IOLoop(c protocol.Client) error {
	...
	for {
		//After 2 unanswered _heartbeat_ responses, nsqd will timeout and forcefully close the client connection
...
func (p *protocolV2) Exec(client *clientV2, params [][]byte) ([]byte, error) {
	...
	case bytes.Equal(params[0], []byte("RDY")): //Update RDY state (indicate you are ready to receive N messages)
	...
	case bytes.Equal(params[0], []byte("DPUB")): //Publish a deferred message to a topic
		return p.DPUB(client, params)
	case bytes.Equal(params[0], []byte("SUB")): //Subscribe to a topic/channel
		return p.SUB(client, params)
	case bytes.Equal(params[0], []byte("NOP")): //客户端如果idle的话，就用这个回复心跳消息
...
func (p *protocolV2) messagePump(client *clientV2, startedChan chan bool) {
	...
	for {
		...
		case subChannel = <-subEventChan:  //表示有订阅事件发生,这里的subChannel就是客户端实际绑定的channel
			// you can't SUB anymore
			subEventChan = nil
		case <-heartbeatChan: //每隔一段时间发送一次心跳
